...
left_cabin = cab |> 
  ...
  mutate(party_ideo = left_right - nat_ideo,
         left_cabin = as.factor(case_when(left_right <= 5 ~ 1, 
                                          left_right > 5 ~ 0))) |>
...
data = elect_vot |> 
  ...
  mutate(var_lab_right = lab_right - lag(lab_right), 
         var_lab_left = lab_left - lag(lab_left),
         prev_left_cab = lag(left_cabin), var_gdp = gdp_per - lag(gdp_per),
         var_death = age_stand_death - lag(age_stand_death),
         var_tot_death = total_death - lag(total_death)) |> 
  ungroup()
```
### Save 
```{r}
